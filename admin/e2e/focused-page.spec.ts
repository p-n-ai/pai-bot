...
test.skip(!validURL || !wrongTokenURL || !expiredURL || !revokedURL, "focused-page Go test server is required");

test("redeems a private focused page after removing the capability fragment", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "A message for Aina" })).toBeVisible();
  await expect(page.getByText("Private goal report")).toBeVisible();
  await expect(page.getByRole("link", { name: "Continue with P&AI" })).toBeVisible();
...
test("shows safe browser errors without revealing private content", async ({ page }) => {
  const cases = [
