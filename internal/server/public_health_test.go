...
func TestPublicHealthFeature(t *testing.T) {
	...
	assertResponse(
		"/health/status",
		http.StatusOK,
		"{\"status\":\"degraded\",\"components\":[{\"id\":\"application\",\"status\":\"operational\"},{\"id\":\"ai_provider\",\"status\":\"unavailable\"}]}\n",
	)
	assertResponse("/healthz", http.StatusOK, `{"status":"ok"}`)
}
...
func TestPublicStatusPageIncludesCoarseAIState(t *testing.T) {
	...
	healthyBody := request()
	for _, expected := range []string{`"status":"ok"`, `"id":"ai_provider"`, `"status":"operational"`} {
...
func TestPublicStatusChecksProviderWithoutSeparateConfiguration(t *testing.T) {
	checkCalls := 0
	handler := NewTopMux(TopMuxOptions{
		PublicHealthEnabled: func() bool { return true },
		AIHealthCheck: func(context.Context) error {
	...
	if rec.Code != http.StatusOK ||
		...
		t.Fatalf("provider status = %d %q", rec.Code, rec.Body.String())
	}
	if checkCalls != 1 {
