...
| `A-W6D30-TEST-1` | Add Playwright-based admin smoke E2E lane: `admin/e2e` tests, Playwright config, `pnpm test:e2e` + `just admin-e2e`, CI/deploy workflow execution, and failure artifact uploads | 🤖 | ✅ | Added `/login` smoke coverage with `@playwright/test` as a local admin dev dependency and wired CI/deploy browser install + artifact upload. |
