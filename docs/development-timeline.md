...
| Task ID | Task | Owner |
|---------|------|-------|
| `P-W1D1-1` | `internal/chat/gateway.go` — InboundMessage, OutboundMessage, Channel interface, Gateway router ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D1-2` | `internal/chat/telegram.go` — Telegram Bot API adapter with long polling, /start handler, markdown message splitting ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D1-3` | `internal/agent/engine.go` — ProcessMessage: load state → build prompt → call AI → save state → return response ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D1-4` | `internal/curriculum/loader.go` — Load topic YAML + teaching notes markdown from filesystem ✅ (assignee: @djakajaya89) | 🤖 |
...
| Task ID | Task | Owner |
|---------|------|-------|
| `P-W1D2-1` | Message persistence: save every exchange to `messages` table with conversation_id, model, tokens ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D2-2` | Event logging: `events` table, log session_started, message_sent, ai_response (non-blocking goroutine) ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D2-3` | Anthropic provider: Claude Messages API implementation, update router for task-based routing ✅ (assignee: @djakajaya89) | 🤖 |
...
| `P-W1D3-2` | `/start` onboarding: create user record, welcome message, ask what they want to study ✅ (assignee: @djakajaya89) | 🤖 |
| `P-W1D3-3` | User lookup by telegram_id in chat flow, auto-trigger /start if new | 🤖 |
| `P-W1D3-4` | Error recovery: retry with backoff, provider fallback chain, friendly error messages ✅ (assignee: @djakajaya89) | 🤖 |
...
| `P-W1D4-2` | Session management (team decision): use rolling compaction + summary for context continuity instead of fixed 30min session split ✅ (assignee: @djakajaya89) | 🤖 |
