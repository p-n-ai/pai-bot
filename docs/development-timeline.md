...
| `A-W5D24-UI-1` | User management page: add `/settings/users` for admin and platform admin with summary cards, active users table, pending invites table, search/filter state, and shared shell surfaces | Bot | Open | Reuse existing invite form patterns and keep cookie-session protection aligned with the shipped auth hardening. |
| `A-W5D24-UI-2` | Invite management workflow: wire `/settings/users` to the live invite issuance flow, expose activation links, and support pending-invite status presentation | Bot | Open | Build on the shipped `/activate` flow and current admin invite issuance support. |
| `A-W5D24-UI-3` | Data export page: add `/export` for admin and platform admin with download actions for students CSV, conversations JSON, and progress CSV | Bot | Open | Frontend page work complements `P-W5D24-2`, which covers the export endpoints themselves. |
| `A-W5D24-UI-4` | Admin navigation update: add role-aware links for `/settings/users` and `/export`, preserving current shell conventions and `no-store` behavior on protected responses | Bot | Open | Keep route protection consistent with the Week 5 cookie-session auth model. |
| `A-W5D24-UI-5` | Frontend coverage for Week 5 admin pages: add tests for `/settings/users`, `/export`, auth redirects, empty states, and failure states | Bot | Open | Required before the new admin pages are considered complete. |
| `A-W5D24-UI-6` | Docs sync for Week 5 admin UI: update admin docs and UI/UX docs to move shipped surfaces from planned to current and keep route/API descriptions aligned | Bot | Open | Update `docs/admin-panel.md` and `docs/admin-panel-uiux.md` in the same task. |
