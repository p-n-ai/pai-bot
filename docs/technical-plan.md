...
| **Migrations** | `goose` | v3 | SQL-based migrations. Each migration is a single timestamped `.sql` file with `-- +goose Up/Down` blocks in `migrations/`. Primary developer entrypoint is `just migrate` (wrapper around goose). |
